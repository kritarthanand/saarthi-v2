...
export function CleanRitualSummary({ tasks, onToggleTask, readOnly = false }: SummaryViewProps) {
  const theme = threadTheme('#CleanRitual');
  ...
  const heroCopy =
      ...
      ? 'Everything clean. Fresh start to the week.'
      : done === 0
        ? 'Clear space, clear mind — start here.'
        : `${done} of ${total} done — keep the momentum.`;
