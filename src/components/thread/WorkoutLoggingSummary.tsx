...
export function WorkoutLoggingSummary({ tasks, onToggleTask, readOnly = false }: SummaryViewProps) {
  const theme = threadTheme('#WorkoutLog');
  ...
  const heroCopy =
      ...
      : done === 0
        ? "Let's get moving."
        : `${done} of ${total} steps done — finish strong.`;
    ...
    <View style={{ paddingHorizontal: 16, paddingVertical: 4, paddingBottom: 24, gap: 14 }}>
      ...
      >
        <Text style={{ fontSize: 11, color: theme.color, fontWeight: '700', letterSpacing: 1, textTransform: 'uppercase' }}>
          workout · {pointsEarned}/{pointsTotal} pts
        </Text>
